...
const GenderView = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const data = await adminAPI.getGenderStats(gender || '');
        setStats(data);
      } catch (error) {
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Gender statistics not found</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{gender} Statistics</h1>
          <p className="text-gray-600 mt-1">Comprehensive view of gender-based performance</p>
        </div>
      </div>
      ...
      <div className="card">
        <h2 className="text-xl font-bold mb-4">Gender Overview</h2>
