...
import { Users, TrendingUp, Award, BarChart3, ArrowLeft } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

const BatchView = () => {
  const { programme, year } = useParams<{ programme: string; year: string }>();
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        if (programme && year) {
          const data = await adminAPI.getBatchStats(programme, year);
          setStats(data);
        }
      } catch (error) {
  ...
  if (!stats) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Batch statistics not found</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{stats?.batch || `${programme} ${year}`} Statistics</h1>
          <p className="text-gray-600 mt-1">Comprehensive view of batch performance</p>
        </div>
      </div>
      ...
      <div className="card">
        <h2 className="text-xl font-bold mb-4">Batch Overview</h2>
